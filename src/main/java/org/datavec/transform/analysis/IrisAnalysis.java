 ...
 *
 *  * /usr/bin/spark-submit --driver-memory 2G --executor-memory 2G --num-executors 2 --executor-cores 2 --queue datateam_spark --conf spark.default.parallelism=80 --class com.liyuncong.learn.quoraquestionparis.algorithm.IrisAnalysis quora-question-pairs-jar-with-dependencies.jar
 * @author Alex Black
 */
...
//        String directory = new ClassPathResource("IrisData/iris.txt").getFile().getParent(); //Normally just define your directory like "file:/..." or "hdfs:/..."
        JavaRDD<String> stringData = sc.textFile("hdfs://nameservice1/tmp/IrisData/iris.txt");
        ...
        CategoricalAnalysis categoricalAnalysis = (CategoricalAnalysis) dataAnalysis.getColumnAnalysis("Species");
        for(Entry<String, Long> entry : categoricalAnalysis.getMapOfCounts().entrySet()) {
